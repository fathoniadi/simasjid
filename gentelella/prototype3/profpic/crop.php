...
class CropAvatar {
  ...
  public function setGambar($src, $data, $file)
  {
    $this -> setSrc($src);
    $this -> setData($data);
    $this -> setFile($file);
  ...
  private function crop($src, $dst, $data) {
    if (!empty($src) && !empty($dst) && !empty($data)) {
      ...
      $dst_img_w = SIZE_IMAGE;
      $dst_img_h = SIZE_IMAGE;
      ...
      if ($result) 
      {
          $sWmImg = "img/watermark.png";
          $aWmImgInfo = getimagesize($sWmImg);
          $rWmImage = imagecreatefrompng($sWmImg);
          $iSrcWidth=SIZE_IMAGE;
          $iSrcHeight=SIZE_IMAGE;
          $iFrameSize=0;

          $rImage = imagecreatetruecolor($iSrcWidth, $iSrcHeight);

          imagecopy($rImage, $dst_img, 0, 0, 0, 0, $iSrcWidth, $iSrcHeight);

          if (is_array($aWmImgInfo) && count($aWmImgInfo)) 
          {
             imagecopy($rImage, $rWmImage, SIZE_IMAGE-$aWmImgInfo[0], $iFrameSize, 0, 0, $aWmImgInfo[0], $aWmImgInfo[1]);
          }

          if (!imagepng($rImage, $dst)) 
          {
...
$crop = new CropAvatar();
$crop->setGambar(isset($_POST['avatar_src']) ? $_POST['avatar_src'] : null,
